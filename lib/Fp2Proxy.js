...
import { Fp2Client } from '../lib/Fp2Client.js'
import { Fp2Discovery } from '../lib/Fp2Discovery.js'
...
class Fp2Proxy extends CommandLineTool {
  ...
  async proxy () {
    for (const id in this.fp2Clients) {
      try {
        await this.fp2Clients[id].disconnect()
      } catch (error) {
    ...
    for (const id in this.config) {
      try {
        if (id.startsWith('00212E')) {
          const sensors = await this.deconzClient.get('/sensors')
  ...
  async getApiKey (...args) {
    ...
    parser
      ...
      .parse(...args)
    const apiKey = await this.deconzClient.getApiKey('fp2-proxy')
    this.print(jsonFormatter.stringify(apiKey))
    this.config[this.bridgeid] = { apiKey }
    if (this.deconzClient.fingerprint != null) {
      this.config[this.bridgeid].fingerprint = this.deconzClient.fingerprint
    }
  ...
  async readConfig () {
    ...
    const text = await readFile(configFile)
  ...
  async writeConfig () {
    ...
    await writeFile(configFile, text, { mode: 0o600 })
  ...
  createDeconzDiscovery () {
    this.deconzDiscovery = new Discovery({ timeout: this.clargs.options.timeout })
  ...
  async createDeconzClient () {
    const { bridgeid, name } = this.gatewayConfig
    if (this.clargs.options.apiKey == null) {
      if (this.config[bridgeid]?.apiKey != null) {
        this.clargs.options.apiKey = this.config[bridgeid].apiKey
    ...
    this.deconzClient
      .on('error', (error) => {
        if (error.request.id !== this.requestId) {
          if (error.request.body == null) {
            ...
            this.log(
              '%s: request %d: %s %s %s', name, error.request.id,
        ...
        if (error.nonCritical) {
          this.warn('%s: request %d: %s', name, error.request.id, error)
        } else {
          this.error('%s: request %d: %s', name, error.request.id, error)
        }
      })
      .on('request', (request) => {
        if (request.body == null) {
          this.debug(
            '%s: request %d: %s %s', name, request.id, request.method, request.resource
          )
          this.vdebug(
            '%s: request %d: %s %s', name, request.id, request.method, request.url
          )
        } else {
          this.debug(
            '%s: request %d: %s %s %s', name, request.id,
      ...
      .on('response', (response) => {
        this.vdebug(
          '%s: request %d: response: %j', name, response.request.id, response.body
        )
        this.debug(
