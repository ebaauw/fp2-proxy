...
class Fp2Proxy extends CommandLineTool {
  ...
  async pair (...args) {
    ...
    parser
      ...
      .parse(...args)
    const fp2Client = await this.createFp2Client(id)
    const pairindData = await fp2Client.pair(setupCode)
    // const accessories = await fp2Client.accessories()
    this.config[id] = { service: fp2Client.service, pairindData }
    this.writeConfig()
  }
  ...
  async createFp2Client (id) {
    ...
    await this.createFp2Discovery()
    const service = await this.fp2Discovery.find(id)
    const fp2Client = new Fp2Client({
      service, pairData: this.config[id].pairData, timeout: this.clargs.options.timeout
    })
    fp2Client
      .on('', () => { this.debug('%s: ...', id) })
    return fp2Client
